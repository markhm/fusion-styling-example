...
window.Vaadin['_vaadintheme_fusionstylingexample_globalCss'] = window.Vaadin['_vaadintheme_fusionstylingexample_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_fusionstylingexample_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_fusionstylingexample_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_fusionstylingexample_globalCss'].push(target);
  }
  if (!document['_vaadintheme_fusionstylingexample_componentCss']) {
    registerStyles(
    ...
    document['_vaadintheme_fusionstylingexample_componentCss'] = true;
  }
